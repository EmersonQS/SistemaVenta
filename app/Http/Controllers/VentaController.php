...
use Illuminate\Http\VentaDetalle;
...
class VentaController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ->addColumn('action', function ($row) {
                    $editButton = Gate::allows('ventas_edit')
                        ? view('components.button-edit', ['id' => $row->id])->render()
                        : '';

                    $deleteButton = Gate::allows('ventas_delete')
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            Producto::updateStock(true, $venta->detalles->toArray()); // Revertir stock anterior
            $venta->detalles()->delete();
            $venta->detalles()->createMany($ventaData['detalles']);
            Producto::updateStock(false, $ventaData['detalles']); // Aplicar nuevo stock
    ...
    public function destroy($id)
    {
        try {
            ...
            Producto::updateStock(true, $registro->detalles->toArray()); // Revertir stock
            $registro->delete();

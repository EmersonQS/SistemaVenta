...
use Illuminate\Http\CompraDetalle;
...
class CompraController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ->addColumn('action', function ($row) {
                    $editButton = Gate::allows('compras_edit')
                        ? view('components.button-edit', ['id' => $row->id])->render()
                        : '';

                    $deleteButton = Gate::allows('compras_delete')
                        ? view('components.button-delete', ['id' => $row->id])->render()
                        : '';
    ...
    {
        try {
            //$registro = Compra::with(['detalles.producto.afectacionTipo', 'cliente'])->findOrFail($id);
            $registro = Compra::with([
                ...
                'proveedor' => function ($query) {
                    $query->select('id', 'numero_documento', 'razon_social');
                }
            ])->findOrFail($id);
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            Producto::updateStock(false, $compra->detalles->toArray()); // Revertir stock anterior
            $compra->detalles()->delete();
            $compra->detalles()->createMany($compraData['detalles']);
            Producto::updateStock(true, $compraData['detalles']); // Aplicar nuevo stock
    ...
    public function destroy($id)
    {
        try {
            ...
            Producto::updateStock(false, $registro->detalles->toArray()); // Revertir stock
            $registro->delete();
